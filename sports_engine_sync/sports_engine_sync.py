...
def create_login_session(
    username: str, password: str, use_mfa: True, session: requests.Session = None
) -> requests.Session:
    ...
    response = _get_and_submit_form(
        session=session,
        url="https://user.sportngin.com/users/sign_in",
        data_callback=_username_callback,
    )
    ...
    if not use_mfa:
        ...
        _submit_form(
            ...
            url=response.url,
            text=response.text,
            data_callback=_login_callback,
        )
    else:

        def _mfa_callback(data):
            data.update(
                {
                    "user[password]": password,
                    "user[login]": username,
                    "mfa_option": "text_a_code",
                }
            )
            ...
            data.update(
                {
                    ...
                    "sms_otp_resend": "0",
                    "commit": "Continue",
                    "user[otp_attempt]": getpass.getpass(prompt="Enter OTP"),
                }
            )
...
_LIST_OPPONENT_URL = (
    "https://api.sportngin.com/v3/teams/%s/opponents?page=1&per_page=100"
)
...
def list_opponents(session, team_id: str):
    ...
    logging.info("list_opponents: %s", url)
...
def list_events(session, team_id: str, end_date: datetime, start_date: datetime):
    url = _LIST_EVENTS_URL % (
        team_id,
        _datetime_to_string(end_date),
        _datetime_to_string(start_date),
    )
...
def create_event_dict(
    team_id: str, start_time: datetime, end_time: datetime, title: str
):
    return dict(
        event_type="event",
        status="scheduled",
...
def create_game_dict(
    ...
    return dict(
        ...
        local_timezone="America/Los_Angeles",
        game_details={
            "team_1": dict(id=team_id, is_home_team=is_home_game, name=team_name),
...
def main():
    ...
    session = create_login_session(
        username=config["username"], password=config["password"], use_mfa=True
    )

    existing_events = list_events(
        session=session,
        team_id=config["teamid"],
        start_date=parse("2023-09-01"),
        end_date=parse("2024-04-01"),
    )
    existing_titles = set([x.get('title') for x in existing_events.get('result')])
    ...
    for event in cal.walk("VEVENT"):
        ...
        if data.get('title') in existing_titles:
            print('skipping existing event ' + data.get('title'))
        else:
            print('Adding new event ' + data.get('title'))
